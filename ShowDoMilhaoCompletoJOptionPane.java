...
public class ShowDoMilhaoCompletoJOptionPane {
    ...
    private static List<RankingEntry> ranking = new ArrayList<>();
    private static final Path RANKING_FILE = Paths.get("ranking.jsonl"); // JSON Lines (um JSON por linha)
    ...
    private static class RankingEntry {
        String nome;
        int premio;
        String dataHora; // ISO string

        RankingEntry(String nome, int premio, String dataHora) {
            this.nome = nome;
            this.premio = premio;
        ...
        String toDisplayString() {
            return nome + " - R$ " + premio + " - " + dataHora;
        }

        String toJsonLine() {
            // Escapa aspas simples e barras
            String safeNome = nome.replace("\\", "\\\\").replace("\"", "\\\"");
            return "{\"name\":\"" + safeNome + "\",\"prize\":" + premio + ",\"date\":\"" + dataHora + "\"}";
        }

        static RankingEntry fromJsonLine(String line) {
            // Line format produced by toJsonLine(): {"name":"...","prize":123,"date":"..."}
            try {
                int n1 = line.indexOf("\"name\":\"");
                if (n1 < 0) return null;
                n1 += 8;
                int n2 = line.indexOf("\"", n1);
                String name = line.substring(n1, n2).replace("\\\"", "\"").replace("\\\\", "\\");
                int p1 = line.indexOf("\"prize\":", n2);
                if (p1 < 0) return null;
                p1 += 8;
                int p2 = line.indexOf(",", p1);
                String prizeStr;
                if (p2 > 0) {
                    prizeStr = line.substring(p1, p2);
                } else {
                    // no comma, find end brace
                    int pb = line.indexOf("}", p1);
                    prizeStr = line.substring(p1, pb);
                }
                int prize = Integer.parseInt(prizeStr.trim());
                int d1 = line.indexOf("\"date\":\"", p2);
                if (d1 < 0) return new RankingEntry(name, prize, "");
                d1 += 8;
                int d2 = line.indexOf("\"", d1);
                String date = line.substring(d1, d2);
                return new RankingEntry(name, prize, date);
            } catch (Exception e) {
                return null;
            }
        }
    ...
    public static void main(String[] args) {
        // Carrega ranking do arquivo ao iniciar
        loadRankingFromFile();
    ...
    public static void iniciarJogo() {
        ...
        while (jogoAtivo && nivelAtual <= 16) {
            ...
            int resultado = fazerPergunta(pergunta);
            // resultado: 1 = acertou, 0 = errou, -1 = desistiu (cancelou)
            if (resultado == 1) {
                if (nivelAtual < 16) {
                ...
                // jogador desistiu (clicou cancelar e confirmou desistir)
                JOptionPane.showMessageDialog(null, "💰 Você desistiu e saiu com: R$ " + premioAtual);
                adicionarAoRanking(nomeJogador, premioAtual);
                jogoAtivo = false;
            } else {
    ...
    // FAZER PERGUNTA
    // retorna: 1 = correto, 0 = incorreto, -1 = desistiu/cancelou (confirmado)
    private static int fazerPergunta(Pergunta pergunta) {
        ...
        if (resposta == null) {
            // Usuário clicou em Cancel -> perguntar se deseja desistir.
            int confirma = JOptionPane.showConfirmDialog(null,
                    "Você clicou em Cancelar. Deseja desistir e ficar com o prêmio atual de R$ " + premioAtual + "?",
                    "Confirmar Desistência",
                    JOptionPane.YES_NO_OPTION);
            if (confirma == JOptionPane.YES_OPTION) {
                return -1; // desistiu
            } else {
                // Não confirma desistir, considerar que não respondeu e tratar como erro (ou poderia re-exibir).
                // Aqui, retornamos 0 para tratar como resposta incorreta.
                return 0;
            }
        }
        ...
        return respostaIndex == pergunta.respostaCorreta ? 1 : 0;
    }

    // RANKING
    public static void exibirRanking() {
        // Carrega ranking atualizado do arquivo
        loadRankingFromFile();
        ...
        if (ranking.isEmpty()) {
            ...
            for (RankingEntry e : ranking) {
                rankingMsg.append(i).append(". ").append(e.toDisplayString()).append("\n");
                i++;
            }
        }
    ...
    public static void adicionarAoRanking(String nome, int premio) {
        String dataHora = LocalDateTime.now().format(DATE_FORMAT);
        RankingEntry nova = new RankingEntry(nome, premio, dataHora);
        // carregar ranking atual
        loadRankingFromFile();
        ranking.add(nova);
        // ordenar por prêmio desc, depois por data (recente primeiro)
        ranking.sort(Comparator.comparingInt((RankingEntry e) -> e.premio).reversed()
                .thenComparing((RankingEntry e) -> e.dataHora).reversed());
        // salvar no arquivo
        saveRankingToFile();
    }
    ...
    public static void exibirInstrucoes() {
        String instrucoes = """
            ...
            • Se clicar em Cancelar na pergunta, será perguntado se deseja desistir (ficar com o prêmio atual)
    ...
    private static void loadRankingFromFile() {
        ...
        try {
            List<String> linhas = Files.readAllLines(RANKING_FILE, StandardCharsets.UTF_8);
            for (String l : linhas) {
                if (l == null || l.trim().isEmpty()) continue;
                RankingEntry e = RankingEntry.fromJsonLine(l.trim());
                if (e != null) ranking.add(e);
            }
            // ordenar por prêmio desc
            ranking.sort(Comparator.comparingInt((RankingEntry e) -> e.premio).reversed()
                    .thenComparing((RankingEntry e) -> e.dataHora).reversed());
        } catch (IOException ex) {
            // Não falhar a aplicação por causa do arquivo; apenas logar via dialog
            JOptionPane.showMessageDialog(null, "Erro ao carregar ranking: " + ex.getMessage(), "Erro", JOptionPane.WARNING_MESSAGE);
        }
    }
    ...
    private static void saveRankingToFile() {
        ...
        for (RankingEntry e : ranking) {
            linhas.add(e.toJsonLine());
        }
        try {
            Files.write(RANKING_FILE, linhas, StandardCharsets.UTF_8);
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(null, "Erro ao salvar ranking: " + ex.getMessage(), "Erro", JOptionPane.WARNING_MESSAGE);
        }
    }
}